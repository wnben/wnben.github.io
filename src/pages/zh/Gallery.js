...
const albums = {
  '枫丹白露宫': FontainebleauImages.map((src, index) => ({
    src,
    thumbnail: FontainebleauThumbnails[index],
    alt: `Fontainebleau ${index + 1}`
  })),
  '枫丹白露宫 ': Fontainebleau2Images.map((src, index) => ({
    src,
    thumbnail: Fontainebleau2Thumbnails[index],
    alt: `Fontainebleau2 ${index + 1}`
  })),
  '埃菲尔铁塔': TourEiffelImages.map((src, index) => ({
    src,
    thumbnail: TourEiffelThumbnails[index],
    alt: `TourEiffel ${index + 1}`
  })),
  '法兰西学院': FranceImages.map((src, index) => ({
    src,
    thumbnail: FranceThumbnails[index],
    alt: `France ${index + 1}`
  })),
  '卢浮宫': LouvreImages.map((src, index) => ({
    src,
